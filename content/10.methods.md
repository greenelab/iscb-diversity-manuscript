...
Excluding 45 authors with empty fore name field, genderize.io fails to provide gender predictions for 3,044 fore names of 34,005 corresponding authors, and two names of 411 ISCB honorees.
Of these names, approximately 45% have no more than three characters which are likely initials (e.g., "AB", "A.B", or "A B"), and 27% have a hyphen and more than three characters.
We note that hyphenated names, most of which are predicted to be of Asian origin by last name (87%), are more unique and thus are more difficult to find predictions for.
...
Excluding 39 corresponding authors with empty last name field, `wru` fails to provide race/ethnicity predictions for 8,810 last names of 34,011 corresponding authors, and 98 names of 411 ISCB honorees.
In addition to NA predictions, another limitation of not only `wru` but other race/ethnicity and nationality prediction methods can be inaccurate prediction for women scientists who changed their last name to their husband's if they have different race/ethnicity.
