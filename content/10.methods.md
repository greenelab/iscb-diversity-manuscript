...
We also applied the Python package `ethnicolr` to estimate race and ethnicity, as developed by Sood and Laohaprapanon. [@arxiv:1805.02109]
...
For instance, 76% of the names in ethnicolr’s Wikipedia dataset are European in origin, and the dataset contains substantially fewer Asian, African, and Middle Eastern names. 
...
To generate a training dataset for nationality prediction, we scraped Wikipedia’s category of Living People (approximately 930,000 pages).
...
Using a union of these strategies, we were able to define a name and nationality for 708,493 people via their Wikipedia pages with high fidelity.
...
We trained a Long Short-term Memory (LSTM) neural network, the same kind of classifier used by the ethnicolr creators.
We trained the classifier to recognize patterns in the sequences of letters in a name to infer region of origin.
