...
These method extract a sequence of textual affiliations for each author.
While ideally each affiliation record would refer to one and only one research organization, sometimes journals deposit multiple affiliations in a single structured affilation.
...
Therefore, we designed a method for extracting countries from affilations that accomodated multiple countries (see [notebook](https://github.com/greenelab/iscb-diversity/blob/96ad84b0c58c3b3de301056f7546664f8bacb057/07.affiliations-to-countries.ipynb).
...
The first, geotext, using regular expressions to find mentions of places from the [GeoNames database](http://www.geonames.org/).
For example, in the above text, geotext detected four mentions of places in Germany: Saarland, Saarbrücken, Saarland, Germany.
