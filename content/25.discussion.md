...
A major technical challenge was to narrowing down geographic origins for some groups of names.
...
A key element is then implementing processes to use these plans to continuously evaluate practices.

A challenge that straddles the ethical and technical divide for studies examining the representation of honorees is that the background that can be best assessed, which we assess here, is the current field composition.
...
To the extent that this is due to systemic barriers, the process may have reached equality but not equity.
