...
We applied the square root transformation to account for the skewed distribution of citation counts and the addition of 1 to avoid dropping any article with no citation.
...
Moreover, although the average of the adjusted probabilities is not exactly "proportion", we use the phrase "estimated proportion" for ease of interpretation.
For example, the average of adjusted probabilities of authors' names to have East Asian origin is the estimate for the proportion of authors with East Asian names.
