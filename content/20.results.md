...
### Ethical Considerations for Representation

A challenge with studies examining the representation of honorees is that the background that can be best assessed, which we assess here, is the current field composition.
Scientific societies exist to promote the discipline, and many, including ISCB, include diversity as a value.
In these cases the ideal background distribution would be the portion of the population that would be senior computational biologists in the absence of systemic barriers to participation.
...
Consequently, systemic barriers that reduce representation within the field as a whole would reduce apparent disparities in honor distributions as long as those systemic barriers did not also have a particular influence on the honoree selection process as well.
The exercise in this manuscript can only identify disparities, but a failure to detect disparities does not mean that systemic barriers are absent.
SENTENCE ABOUT HAS NOT YET REACHED PARITY HERE.
DISCUSSION OF ONLY CONSIDERING TWO GENDERS HERE.
