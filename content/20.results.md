...
A log fold change probability confusion heatmap shows high multi-region classification accuracy compared to chance (C). The mean probability value in each predicted region is divided by the number of true names in that region (scaled to expected accuracy) to account for the imbalance of regions in the testing dataset.
