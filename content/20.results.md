...
We expected this approach to be overly conservative, leading to a fail to reject the null hypothesis of no disparities even if disparities existed.
...
An alternative approach that considered each pair of forename and last name as the unit of measurement showed similar results.
In addition, if we assume that honors accrue 10 years after their most prolific year with respect to authorships,
a 10-year lag model results in a similar underrepresentation of East Asian scientists' names in the group of honorees, though the effect size is less striking.
...
Please see the [analysis notebook](https://greenelab.github.io/iscb-diversity/11.name-origin.html) for more details.
