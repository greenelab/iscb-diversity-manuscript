...
The confusion heatmap of log fold change of predicted over expected probability shows high multi-region classification accuracy compared to random chance (Fig. {@fig:wiki2019_lstm}C).
...
Calibration curve, computed with the caret R package, shows consistency between the predicted probabilities (midpoints of each fixed-width bin) and the observed fraction of names in each bin (B).
Log fold change probability confusion heatmap shows high multi-region classification accuracy compared to chance (C). Mean probability value in each predicted region is divided by the number of true names in that region (scaled to expected accuracy) to account for the imbalance of regions in the testing dataset.
