...
### Evaluation Set and Field-specific Background
...
With insightful feedback from the reviewers, after multiple iterations of the analysis (see Methods), we extended the analysis to all 176,110 computational biology PubMed articles, extracted names of last authors, and analyzed the raw prediction probability values to capture the uncertainty of the prediction model. (Fig. {@fig:workflow}).
We assumed that, among the authors of a specific paper, the last author (often research advisors) would be most likely to be invited for a keynote or honored as a fellow, and so comparing honors to last authorships would be appropriate.

This study focuses on the field of computational biology.
However, our analysis framework can be adapted to other fields if the researcher curates their own set of honors and replaces the MeSH term "computational biology" with their topic of interest.
While PubMed contains the most comprehensive resource of biomedical texts and associated standardized metadata of which we are aware, a difference database like [arXiv](http://arxiv.org/) may be more appropriate for extracting articles in other fields.

The units in this analysis are honors and authorships, not individuals.
...
Our findings across different versions of the study were consistent with respect to the broad conclusions, though the numerical results differed (see Methods and supplementary figure S3).
...
](https://raw.githubusercontent.com/greenelab/iscb-diversity/d84aa95ea02fff615c89668a20a957298df855ed/figs/workflow.png){#fig:workflow width="90%"}
