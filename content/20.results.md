...
We also extracted names of last authors instead of corresponding authors to better capture the scientist population that is most likely to be invited for keynotes or to be honored as fellows.
...
However, we acknowledged that we might have miss corresponding authors in the first position who can be considered for keynote speakers or fellows.
...
Previously, without citation counts, we found even more pronounced differences between the two groups of authors and honorees.
...
In recent years, ISCB Fellows and keynote speakers appear to have similar gender proportion compared to the population of authors published in computational biology and bioinformatics journals (averaged around 30% in the last three years, Fig. {@fig:gender_breakdown}, right); however, it has not yet reached parity.
