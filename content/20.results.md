...
There is a slow increasing trend of the proportion of female authors over the years, arriving at just over 20% on average in 2019 (Fig. {@fig:gender_breakdown}A).
Analyzing each journal individually, we observe very similar trend across the three selected journals, but the least increase in the estimated female proportion is found in PLOS Computational Biology (Supplementary Fig S1).
ISCB fellows and keynote speakers appear to be more evenly split between men and women compared to the population of authors published in computational biology and bioinformatics journals (Fig. {@fig:gender_breakdown}B).
The overall increasing female proportion trend seems to be present in each honor category, especially in the group of ISCB Fellows (Fig. {@fig:gender_breakdown}C).
It is more difficult to make inference from the group of PSB speakers because the conference only has 1-2 keynotes each year (Fig. {@fig:gender_breakdown}C).
